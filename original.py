...
if __name__ == "__main__":
    ...
    for page_id, page in enumerate(ocr_results.pages):
        ...
        for line in page.lines:
            ...
            desired_text_width = (
                max(
                    dist(line.polygon[0], line.polygon[1]),
                    dist(line.polygon[3], line.polygon[2]),
                )
                * scale
            )
            desired_text_height = (
                max(
                    dist(line.polygon[1], line.polygon[2]),
                    dist(line.polygon[0], line.polygon[3]),
                )
                * scale
            )
            font_size = desired_text_height
            actual_text_width = pdf_canvas.stringWidth(
                line.content, default_font, font_size
            )
            ...
            text.setTextTransform(
                ...
                math.sin(text_angle),
                math.cos(text_angle),
                line.polygon[3].x * scale,
                page_height - line.polygon[3].y * scale,
            )
            text.setHorizScale(desired_text_width / actual_text_width * 100)
            text.textOut(line.content + " ")
